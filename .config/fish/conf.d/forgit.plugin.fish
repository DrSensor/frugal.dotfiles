...
set -g forgit_pager        (git config core.pager || echo 'cat')
set -g forgit_show_pager   (git config pager.show || echo "$forgit_pager")
set -g forgit_diff_pager   (git config pager.diff || echo "$forgit_pager")
set -g forgit_ignore_pager "bat -l gitignore --color=always"
set -g forgit_log_format   "-%C(auto)%h%d %s %C(black)%C(bold)%cr%Creset"
set -g forgit_emojify '|emojify'
